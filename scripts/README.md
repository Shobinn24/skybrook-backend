...
- `cron_run_refresh_ad_spend.mjs` — primary afternoon ad-spend-refresh scheduler, run by **Railway native cron** at 16:00 UTC. Calls `/api/cron/refresh-ad-spend` (re-pulls ad-spend + fb-ads, runs the FB-Tracker-2 append) and awaits the (~10s) synchronous response. Made primary 2026-06-03 because the GH workflow fires 30min-2hr late, delaying the tracker-2 append to ~1:30-2pm ET. Needs `CRON_SECRET`; reads `APP_URL`/`RAILWAY_PUBLIC_DOMAIN`. Same setup as the ingest cron (see below) but schedule `0 16 * * *` and this script. `.github/workflows/cron-refresh-ad-spend.yml` stays as an idempotent backstop.
